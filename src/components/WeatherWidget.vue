...
const temperature = ref<number>(0) // Temperature in Celsius
const cityName = ref<string>('') // City name
const windSpeed = ref<number>(0) // Wind speed in km/h
const humidity = ref<number>(0) // Humidity percentage
const weatherDescription = ref<string>('') // Weather description
const bgImageUrl = ref<string>('') // Background image URL
const currentIcon = ref<any>(Sun) // Default icon for sunny weather
...
const updateIcon = (weatherCondition: string) => {
  if (weatherCondition.includes('Clear')) {
